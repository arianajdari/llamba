#ifndef ___SUMMATION_HPP___
#define ___SUMMATION_HPP___
...
class summation : protected execution_time_calculation {
    public:
        RESULT* execute_serial(int data_size, int iteration_number) 
        {
            ...
            for(int i = 0; i < iteration_number; i++)
            {
                ...
                llamba::base::sum_parallel<double> sum = llamba::base::sum_parallel<double>(matrix_a, matrix_b, result, settings_serial);
        ...
        RESULT* execute_openmp(int data_size, int iteration_number, int thread_number) 
        {
            ...
            for(int i = 0; i < iteration_number; i++)
            {
                auto result  = llamba::single_generator::generate_input_zero<double>(data_size);
                auto before = std::chrono::high_resolution_clock::now();
                llamba::base::sum_parallel<double> sum = llamba::base::sum_parallel<double>(matrix_a, matrix_b, result, settings_openmp);
                auto after = std::chrono::high_resolution_clock::now();
        ...
        RESULT* execute_llamba(int data_size, int iteration_number, int thread_number, int scheduling_strategy, int priority, bool affinity) 
        {
            ...
            for(int i = 0; i < iteration_number; i++)
            {
                auto result  = llamba::single_generator::generate_input_zero<double>(data_size);
                auto before = std::chrono::high_resolution_clock::now();
                llamba::base::sum_parallel<double> sum = llamba::base::sum_parallel<double>(matrix_a, matrix_b, result, settings_);
                auto after = std::chrono::high_resolution_clock::now();

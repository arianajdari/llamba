...
int main() 
{
    ...
    for(int i = 0; i < ITERATION_NUMBER; i++)
    {
        ...
        time_t start = clock();
        llamba::base::abstract_parallel<double> multiply = llamba::base::abstract_parallel<double>(matrix_a, matrix_b, result, settings_serial);
    ...
    for(int i = 0; i < ITERATION_NUMBER; i++)
    {
        auto result  = llamba::single_generator::generate_input_zero<double>(DATA_SIZE);
        time_t start = clock();
        llamba::base::abstract_parallel<double> multiply = llamba::base::abstract_parallel<double>(matrix_a, matrix_b, result, settings_);
    ...
    std::cout << "Abstraction Benchmark -- Matrix Size: (" << DATA_SIZE << "x" << DATA_SIZE << ")" << std::endl << std::endl;

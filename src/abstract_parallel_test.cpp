#include <llamba/base/abstract_parallel.hpp>
...
int main()
{
  ...
  abstract_serial(matrix_a, matrix_b, result_serial);

  llamba::base::abstract_parallel<DATA_TYPE> abstract_parallel_ = 
    llamba::base::abstract_parallel<DATA_TYPE>(matrix_a, matrix_b, result_parallel, llamba::settings::make_settings());
  
  if(compare_results(result_serial, result_parallel))
    std::cout << "Serial Abstraction and Parallel Abstraction are equal" << std::endl;
  else
    std::cout << "Serial Abstraction and Parallel Abstraction are not equal" << std::endl;

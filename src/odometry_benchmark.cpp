...
//////////////////////////////////////////////////////
  ...
  for(int i = 0; i < 10; i++) {
    times.push_back(odometryUpdate(pf, pose, delta, ITERATIONS_100000));
    std::this_thread::sleep_for(std::chrono::operator""ms(500));
  }

  calculate(times, ITERATIONS_100000, "Single Threaded");

  times.clear();

  for(int i = 0; i < 10; i++) {
    times.push_back(odometryUpdateMultithreading(pf, pose, delta, ITERATIONS_100000));
    std::this_thread::sleep_for(std::chrono::operator""ms(500));
  }

  calculate(times, ITERATIONS_100000, "Multi Threaded");
////////////////////////////////////////////////////////////

  times.clear();

  for(int i = 0; i < 10; i++) {
    times.push_back(odometryUpdate(pf, pose, delta, ITERATIONS_1000000));
    std::this_thread::sleep_for(std::chrono::operator""ms(500));
  }

  calculate(times, ITERATIONS_1000000, "Single Threaded");

  times.clear();

  for(int i = 0; i < 10; i++) {
    times.push_back(odometryUpdateMultithreading(pf, pose, delta, ITERATIONS_1000000));
    std::this_thread::sleep_for(std::chrono::operator""ms(500));
  }

  calculate(times, ITERATIONS_1000000, "Multi Threaded");

/////////////////////////////////////////////////////////////////////////////
  times.clear();

  for(int i = 0; i < 10; i++) {
    times.push_back(odometryUpdate(pf, pose, delta, ITERATIONS_3000000));
    std::this_thread::sleep_for(std::chrono::operator""ms(500));
  }

  calculate(times, ITERATIONS_3000000, "Single Threaded");

  times.clear();

  for(int i = 0; i < 10; i++) {
    times.push_back(odometryUpdateMultithreading(pf, pose, delta, ITERATIONS_3000000));
    std::this_thread::sleep_for(std::chrono::operator""ms(500));
  }

  calculate(times, ITERATIONS_3000000, "Multi Threaded");
//////////////////////////////////////////////////////////////////////////////   

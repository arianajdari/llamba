#include <llamba/base/green_channel_parallel.hpp>
...
void green_channel_serial(const std::vector<DATA_TYPE>& matrix_a, const std::vector<DATA_TYPE>& matrix_b, std::vector<DATA_TYPE>& result_serial)
{
...
int main()
{
  ...
  green_channel_serial(matrix_a, matrix_b, result_serial);

  llamba::base::green_channel_parallel<DATA_TYPE> green_channel_parallel_ = 
    llamba::base::green_channel_parallel<DATA_TYPE>(matrix_a, matrix_b, result_parallel, llamba::settings::make_settings());
  
  if(compare_results(result_serial, result_parallel))
    std::cout << "Serial Green Channel and Parallel Green Channel are equal" << std::endl;
  else
    std::cout << "Serial Green Channel and Parallel Green Channel are not equal" << std::endl;

...
int main()
{
  ...
  sum_serial(matrix_a, matrix_b, result_serial);

  llamba::base::sum_parallel<DATA_TYPE> sum_parallel_ = 
    llamba::base::sum_parallel<DATA_TYPE>(matrix_a, matrix_b, result_parallel, llamba::settings::make_settings());
  
  if(compare_results(result_serial, result_parallel))
    std::cout << "Serial Summation and Parallel Summation are equal" << std::endl;
  else
    std::cout << "Serial Summation and Parallel Summation are not equal" << std::endl;
